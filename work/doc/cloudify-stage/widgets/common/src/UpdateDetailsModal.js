...
function BlueprintSection(props) {
    ...
    return (
        <React.Fragment>
            ...
            {
                isChanged
                    ? <span>Changed from <strong>{props.oldBlueprint}</strong> into <strong>{props.newBlueprint}</strong>.</span>
...
class InputsSection extends React.Component {
    ...
    render() {
        let {Form, Header, Icon, List, ParameterValue, ParameterValueDescription, Popup, PopupHelp, Table} = Stage.Basic;
        let {Json} = Stage.Utils;

        const newInputs = Array.sort(_.keys(this.props.newInputs));
        const onlyChangedInputs = _.chain(newInputs)
            .filter((inputName) => !_.isEqual(Json.getStringValue(this.props.newInputs[inputName] || ''),
...
function NodeInstancesCard(props) {
    let {Card, Icon, Label, List} = Stage.Basic;
...
function NodeInstancesSection(props) {
    let {Card, Header, PopupHelp} = Stage.Basic;
...

            {
                isChanged
                    ...
                    <Card.Group itemsPerRow={2}>
                        {_.map(props.types, (type) => <NodeInstancesCard key={type.name} {...type} />)}
                    </Card.Group>
                    :
        ...
        <React.Fragment>
            <Header>
                Steps
                {
                    stepsPresent &&
                    <Header.Subheader>
                        Action steps taken during update.
                    </Header.Subheader>
                }
            </Header>
            {
...
export default class UpdateDetailsModal extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        if (!_.isEmpty(this.props.deploymentUpdateId) && !prevProps.open && this.props.open) {
            ...
            actions.doGetUpdate(this.props.deploymentUpdateId)
                .then((deploymentUpdate) => {
                    if (_.isEmpty(this.props.executionParameters) && !_.isEmpty(deploymentUpdate.execution_id)) {
                        actions.doGetExecutionParameters(deploymentUpdate.execution_id)
                            .then(({parameters : executionParameters}) => this.setState({loading: false, error: null, deploymentUpdate, executionParameters}))
                            .catch((err) => this.setState({loading: false, error: err.message, deploymentUpdate: UpdateDetailsModal.EMPTY_DEPLOYMENT_UPDATE}));
                    } else {
                ...
                <Modal open={this.props.open} onClose={()=>this.props.onClose()} className='updateDetailsModal'>
                    ...
                    <Modal.Content scrolling>
                        <Form loading={this.state.loading} errors={this.state.errors}
                            ...
                            <BlueprintSection oldBlueprint={deploymentUpdate.old_blueprint_id}
                                              newBlueprint={deploymentUpdate.new_blueprint_id} />

                            <InputsSection oldInputs={deploymentUpdate.old_inputs}
                                           newInputs={deploymentUpdate.new_inputs} />

                            <NodeInstancesSection types={[
                                {
                                    name: 'add', icon: 'plus', color: 'green', action: 'Install',
                                    workflowSkipped: executionParameters.skip_install,
                                    instances: this.getInstances(deploymentUpdate, 'added')
                                },
                                {
                                    name: 'remove', icon: 'minus', color: 'red', action: 'Uninstall',
                                    workflowSkipped: executionParameters.skip_uninstall,
                                    instances: this.getInstances(deploymentUpdate, 'removed')},
                                {
                                    name: 'modify', icon: 'edit', color: 'teal', action: 'Reinstall',
                                ...
                                {
                                    name: 'explicit_reinstall', icon: 'sync', color: 'blue',
