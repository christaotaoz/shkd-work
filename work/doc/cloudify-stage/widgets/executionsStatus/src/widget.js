...
Stage.defineWidget({
    ...
    render: function(widget, data, error, toolbox) {
        ...
        if (_.isEmpty(data.items)) {
            ...
            return <Message content='当前没有可执行。'/>;
        }

        const formatted_data = _.sortBy(_.map(data.items, (status_sum) =>
            ({'status': _.startCase(status_sum.status_display), 'number_of_executions': status_sum.executions})),
        ...
        return (<Graph type={Graph.BAR_CHART_TYPE}
                       data={formatted_data}
                       charts={charts}
                       xDataKey='status'
