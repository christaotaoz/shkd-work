...
export default class MagicModal extends React.Component {

    constructor(props,context) {
        super(props,context);

        this.state = MagicModal.initialState;
    ...
    static _doExecute (toolbox, deployment, workflow, force=true) {
    ...
    static waitCustom(toolbox,deploymentName){

        MagicModal._doExecute(toolbox,deploymentName,'install')
    }

    componentDidUpdate(prevProps) {
        if (!prevProps.open && this.props.open) {
            this.setState(MagicModal.initialState);
        }
    }
    ...
    _submitDeploy () {
        ...
        actions.doDeploy(this.state.blueprint, this.state.deploymentName, deploymentInputs, this.state.visibility, this.state.skipPluginsValidation)
            .then((/*deployment*/)=> {
                ...
                // MagicModal.waitCustom(toolbox,this.state.deploymentName);
                let deployments = toolbox.getManager().doGet('/deployments?_include=id');
                console.log('看看部署有啥');
                console.log(deployments);
                console.log(this.state.deploymentName);
                // setTimeout(MagicModal.waitCustom,5000,...[toolbox,this.state.deploymentName])
            })
            .catch((err)=>{
        ...
        return (
            <Modal open={this.props.open} onClose={()=>this.props.onHide()}>
                ...
                <Modal.Actions>
                    ...
                    <ApproveButton onClick={this.onApprove.bind(this)} disabled={this.state.loading} content="创建部署并执行工作流" icon="rocket" className="green"/>
                </Modal.Actions>
            </Modal>
        );
    }
