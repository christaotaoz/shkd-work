                    ...
                    this.props.data.items.map((item)=>{
                        ...
                        return (
                            <DataTable.Row key={item.id} selected={item.isSelected} onClick={()=>this.props.onSelect(item)}>
                                <DataTable.Data><Image src={Stage.Utils.Url.url(item.image_url)} width="30px" height="auto" inline/> <a href={item.html_url} target="_blank">{item.name}</a></DataTable.Data>
                                <DataTable.Data>{item.description}</DataTable.Data>
                                <DataTable.Data>{item.created_at}</DataTable.Data>
                                <DataTable.Data>{item.updated_at}</DataTable.Data>
                                <DataTable.Data className="center aligned rowActions">
                                    <Icon name={isLoading ? 'spinner' : 'info'} link={!isLoading} title="模板描述s"
                                          loading={isLoading} bordered={!isLoading}
                                          onClick={(event)=>{event.stopPropagation();this.props.onReadme(item.name, item.readme_url)}} />
                                    <Icon name="upload" link title="上传模板" bordered
