...
export default class EventFilter extends React.Component {
    ...
    render () {
        let {Form, Popup, TimeFilter} = Stage.Basic;
        const {EventUtils} = Stage.Common;
        ...
        return (
            <Form size="small">
                <Form.Group inline widths='equal'>
                    <Form.Field>
                        <Form.Dropdown placeholder='类型' fluid selection options={EventUtils.typesOptions} name="type"
                                       value={this.state.fields.type} onChange={this._handleInputChange.bind(this)}/>
                    </Form.Field>
                    <Form.Field>
                        <Form.Dropdown placeholder='事件类型' fluid multiple search selection options={this.state.eventTypeOptions}
                                       name="eventType" renderLabel={this._renderLabel.bind(this)}
                                       additionLabel='Add custom Event Type: '
                                       value={this.state.fields.eventType} allowAdditions disabled={!this._isTypeSet(EventUtils.eventType)}
                                       onAddItem={this._handleOptionAddition.bind(this)}
                                       onChange={this._handleInputChange.bind(this)}/>
                    </Form.Field>
                    <Form.Field>
                        <Form.Dropdown placeholder='日志级别' fluid multiple search selection options={this.state.logLevelOptions}
                                       name="logLevel" allowAdditions disabled={!this._isTypeSet(EventUtils.logType)}
                    ...
                    <Form.Field>
                        <Popup trigger={<Form.Button icon="undo" basic onClick={this._resetFilter.bind(this)} disabled={!this._isDirty()} />}
                               content='Reset filter' />
                    </Form.Field>
                </Form.Group>
                <Form.Group inline widths='equal'>
                    <Form.Input placeholder='操作' name="operationText" fluid
                                value={this.state.fields.operationText} onChange={this._handleInputChange.bind(this)} />
                    <Form.Input placeholder='信息' name="messageText" fluid
