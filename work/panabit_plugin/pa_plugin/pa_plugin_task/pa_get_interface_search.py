...
def wan_interface(ip_port,cookies,license_id):
    ...
    if not num_L:
        ...
        cur = conn.cursor()
        cur.execute('select ip,line_name,licence_id from wan_interface;')
        L = cur.fetchall()
        ...
        for i in range(Counts):
            if i >= 1:
                ...
                if not L:
                        cur.execute(
                            "insert into wan_interface(line_name,network_card,state,ip,gateway,mtu,vlan,dns_traction_failure_rate,Inflow_rate,outflow_rate,connects,annotate,licence_id) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
                            Value[13 * i + 1], Value[13 * i + 2], Value[13 * i + 3], Value[13 * i + 4],
                            Value[13 * i + 5], Value[13 * i + 6], Value[13 * i + 7], Value[13 * i + 8], Value[13 * i + 9],
                            Value[13 * i + 10], Value[13 * i + 11], Value[13 * i + 12],license_id))
                        conn.commit()
                else:
                    L = [str(x) for x in L]
                    if "('%s', '%s', '%s')"%(str(Value[13 * i + 4]),str(Value[13 * i + 1]),str(license_id)) in L:
                        cur.execute(
                            "update wan_interface set line_name='%s',network_card='%s',state='%s',ip='%s',gateway='%s',mtu='%s',vlan='%s',dns_traction_failure_rate='%s',Inflow_rate='%s',outflow_rate='%s',connects='%s',annotate='%s',licence_id='%s' where ip='%s' and licence_id='%s' and line_name='%s';" % (
                                Value[13 * i + 1], Value[13 * i + 2], Value[13 * i + 3],
                                Value[13 * i + 4],
                                Value[13 * i + 5], Value[13 * i + 6], Value[13 * i + 7], Value[13 * i + 8],
                                Value[13 * i + 9], Value[13 * i + 10], Value[13 * i + 11], Value[13 * i + 12],
                                license_id, Value[13 * i + 4], license_id,Value[13 * i + 1]))
                        conn.commit()
                    else:
                        cur.execute(
                            "insert into wan_interface(line_name,network_card,state,ip,gateway,mtu,vlan,dns_traction_failure_rate,Inflow_rate,outflow_rate,connects,annotate,licence_id) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
...
def lan_interface(ip_port,cookies,license_id):
    ...
    if not num_L:
        ...
        cur.execute('select ip,interface_name,licence_id from lan_interface;')
        L = cur.fetchall()
        for i in range(Counts):
            if i >= 1:
                if Value[10 * i + 3] == '1':
                ...
                if Value[10 * i + 3] == '0':
                    Value[10 * i + 3] = 'off'
                if not L:
                    cur.execute("insert into lan_interface(interface_name,network_card,state,ip,network_mask,mtu,vlan,inflow_rate,outflow_rate,licence_id) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (Value[10 * i + 1], Value[10 * i + 2], Value[10 * i + 3], Value[10 * i + 4],Value[10 * i + 5], Value[10 * i + 6], Value[10 * i + 7], Value[10 * i + 8], Value[10 * i + 9],license_id))
                    conn.commit()
                else:
                    L = [str(x) for x in L]
                    if "('%s', '%s', '%s')"%(Value[10 * i + 4],Value[10 * i + 1],license_id) in L:
                        cur.execute(
                            "update lan_interface set interface_name='%s',network_card='%s',state='%s',ip='%s',network_mask='%s',mtu='%s',vlan='%s',inflow_rate='%s',outflow_rate='%s',licence_id='%s' where ip='%s' and licence_id='%s' and interface_name='%s';" % (
                                Value[10 * i + 1], Value[10 * i + 2], Value[10 * i + 3],
                                Value[10 * i + 4], Value[10 * i + 5], Value[10 * i + 6], Value[10 * i + 7],
                                Value[10 * i + 8], Value[10 * i + 9], license_id, Value[10 * i + 4], license_id,Value[10 * i + 1]))
                        conn.commit()
                    else:
                        cur.execute(
                            "insert into lan_interface(interface_name,network_card,state,ip,network_mask,mtu,vlan,inflow_rate,outflow_rate,licence_id) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
                            Value[10 * i + 1], Value[10 * i + 2], Value[10 * i + 3],
                            Value[10 * i + 4], Value[10 * i + 5], Value[10 * i + 6], Value[10 * i + 7],
                            Value[10 * i + 8], Value[10 * i + 9], license_id))
                        conn.commit()
...
def setStateOff():
    conn = psycopg2.connect(database='cloudify_db', user='cloudify', password='cloudify', host='127.0.0.1', port='5432')
    cur = conn.cursor()
    cur.execute('select licence_id from lan_interface;')
    L = cur.fetchall()
    if L:
        cur.execute("update lan_interface set state='off';")
        conn.commit()
    cur.execute('select licence_id from wan_interface;')
    L = cur.fetchall()
    if L:
        cur.execute("update wan_interface set state='off';")
        conn.commit()
    conn.close()
